...
- **FR-10.7.4:** Runtime layer owns only execution-environment primitives.
  - Rationale: the `node` runtime layer should contain only what is true of Node.js regardless of language, framework, or package manager. `docker-compose.dev.yml` stays here but is made package-manager-agnostic via a bridge script.
  - Acceptance Criteria:
...
- **FR-10.7.6:** Package-manager layers own manager-specific scaffold artifacts including the `start.sh` bridge script.
  - Rationale: `docker-compose.dev.yml` must be stable and package-manager-agnostic (owned by the runtime layer); `start.sh` is the single package-manager-specific seam that the compose file calls.
  - Acceptance Criteria:
...
- `docker-compose.dev.yml` is stable and owned by the runtime layer; it calls `sh start.sh` as the only package-manager-specific seam.
...
- **`docker-compose.dev.yml` ownership and package-manager isolation:** The compose file lives in `base/node` and is stable across package-manager choices. It calls `sh start.sh`. Each package-manager layer contributes `start.sh` with its own install+dev command. This prevents shared ownership of the compose file without package-manager-specific content.
