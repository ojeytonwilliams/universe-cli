...
| RLB-002 | target environment    | A reduced rollback target set with `production` as the default is sufficient for the spike                                                    | Keeps scope small while exercising command-specific rollback behavior              | Validate with argument parsing tests, success-path tests, and assumptions review                     | validated | Additional target environments can be added when real provider contracts are defined |
...
| LST-002 | environment scope | A reduced environment set with `preview` as the default is sufficient for the spike                                                            | Keeps scope small while exercising command-specific list retrieval behavior        | Validate with argument parsing tests, success-path tests, and assumptions review                 | validated | Confirmed: `preview` default tested; `staging` rejected with exit 6                          |
...
- Evidence links / artifacts:
  - [x] Test(s): `src/adapters/stub-list-client.test.ts` (6 tests), `src/cli.list.test.ts` (13 tests), `src/list.e2e.test.ts` (2 tests), `src/container.test.ts` (guard)
...
- Outcome summary: All LST-001–005 assumptions validated. List follows the identical manifest-first pattern as `logs`, `status`, `rollback`, `promote`, and `deploy`. Stub is deterministic and snapshot-friendly. Two new assumptions (LST-N01, LST-N02) captured around response shape and type boundary choices.
...
| TDN-002 | environment scope     | A reduced environment set with `preview` as the default is sufficient for the spike                                                                | Keeps scope small while exercising command-specific teardown behavior              | Validate with argument parsing tests, success-path tests, and assumptions review                     | validated | Confirmed: `preview` default tested; `staging` rejected with exit 6                                      |
...
- Evidence links / artifacts:
  - [x] Test(s): `src/adapters/stub-teardown-client.test.ts` (6 tests), `src/cli.teardown.test.ts` (13 tests), `src/teardown.e2e.test.ts` (2 tests), `src/container.test.ts` (guard)
...
- Outcome summary: All TDN-001–005 assumptions validated. Teardown follows the identical manifest-first pattern as all other promoted commands. Stub is deterministic and snapshot-friendly. Two new assumptions (TDN-N01, TDN-N02) captured around receipt shape and the completion of the full command surface.
